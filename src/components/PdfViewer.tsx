  ...
  useEffect(() => {
    ...
    async function loadDocument() {
      try {
        ...
        console.log(`[PdfViewer] Resolving URL:`, url);
        ...
        if (isCapacitorNative) {
          console.log(`[PdfViewer] Detected Capacitor Native platform.`);
        }

        // 2. Ensure PDF.js engine is ready
        if (!pdfjsLoaded) {
          if (pdfjsLoadError) {
            throw new Error(`PDF engine load error: ${pdfjsLoadError}`);
          }
          setStatusText("Initializing PDF engine...");
          return;
        }

        // 3. Cache Storage API check for instant offline re-access
        ...
        if (cacheSupported) {
          try {
            ...
            if (cachedResponse) {
              console.log(`[PdfViewer Cache] Loaded cached blob for:`, url);
        ...
        if (!pdfBlob) {
                ...
                xhr.onload = () => {
                  ...
                  if ((status >= 200 && status < 300) || status === 0) {
                    if (xhr?.response && xhr.response.size > 0) {
                      ...
                      reject(new Error("Received empty response from storage."));
                    }
                  } else {
                    const reader = new FileReader();
                    reader.onload = () => {
                      console.error(`[PDF LOAD DIAGNOSTICS] Failed response body:`, reader.result);
                    };
                    if (xhr?.response) {
          ...
          // Save to Cache Storage asynchronously for HTTP(S) links
          if (cacheSupported && pdfBlob && active && !dlUrl.startsWith("data:") && !dlUrl.startsWith("blob:")) {
            try {
              const cache = await caches.open("student-pdf-cache");
              await cache.put(
                url,
